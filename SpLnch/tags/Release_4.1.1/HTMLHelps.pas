...
{       HTML Help interface unit for Borland Delphi4.0               }
{                                                                    }
{            1999.8.15 íÜñÏÅ@òaïF mailto : nakanon@tke.att.ne.jp     }
{                                                                    }
{ HtmlHelp.h                                                         }
...
type
  ...
  tagHHN_NOTIFY = record
    hdr    : NMHDR;
    pszUrl : PAnsiChar; // Multi-byte, null-terminated string
  end;
  THHN_Notify = tagHHN_NOTIFY;
...
type
  ...
  {$EXTERNALSYM tagHH_LAST_ERROR}
  tagHH_LAST_ERROR = record
    cbStruct    : Integer;    // size of this structure
    hr          : HRESULT;    // Specifies the last error code.
    description : WideString; // Specifies a Unicode string containing a description of the error.
  end;
  THH_Last_Error = tagHH_LAST_ERROR;
  ...
  THH_Set_InfoType = tagHH_SET_INFOTYPE;
...
type
  ...
  TFNHtmlHelpW = function(hwndCaller: HWND; pszFile: PWideChar; uCommand: UINT; dwData: DWORD): HWND stdcall;
...
function HtmlHelp(hwndCaller: HWND; pszFile: PAnsiChar; uCommand: UINT; dwData: DWORD): HWND;
function HtmlHelpInstalled: Boolean;

var
  hhctrl : String = 'hhctrl.ocx';

implementation

var
  HHHandle : HModule = 0;
  HHA : TFNHtmlHelpA = nil;
  HHW : TFNHtmlHelpW = nil;
  HH  : TFNHtmlHelpA = nil;

const
// Use the following for GetProcAddress to load from hhctrl.ocx
  ATOM_HTMLHELP_API_ANSI    = PAnsiChar( DWORD( WORD( 14 ) ) );
  ATOM_HTMLHELP_API_UNICODE = PAnsiChar( DWORD( WORD( 15 ) ) );
...
begin
  Result := 0;
  if Assigned(HHA) then
    Result := HHA(hwndCaller, pszFile, uCommand, dwData);
end;

function HtmlHelpW;
begin
  Result := 0;
  if Assigned(HHW) then
    Result := HHW(hwndCaller, pszFile, uCommand, dwData);
end;
...
initialization
begin
  HHHandle := LoadLibrary(PChar(hhctrl));
  if HHHandle <> 0 then begin
    HH  := GetProcAddress(HHHandle, 'HtmlHelpA');
    HHA := GetProcAddress(HHHandle, 'HtmlHelpA');
    HHW := GetProcAddress(HHHandle, 'HtmlHelpW');
  end;
end;

finalization
  if HHHandle <> 0 then FreeLibrary(HHHandle);
end.
